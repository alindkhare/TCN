...
class ElaticTemporalBlock1(nn.Module):
    ...
    def get_active_filter(self, conv_layer):
        return _weight_norm(
            conv_layer.weight_v,
            conv_layer.weight_g,
            0,
    ...
    def forward(self, x):
        # out = self.net(x)
        # out = x
        out = self.apply_conv(self.conv1, x)
        out = self.chomp1(out)
        out = self.relu1(out)
        out = self.dropout1(out)

        out = self.apply_conv(self.conv2, out)
        out = self.chomp2(out)
        out = self.relu2(out)
...
class ElasticTemporalConvNet1(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2):
        ...
        for i in range(num_levels):
            ...
            in_channels = num_inputs if i == 0 else num_channels[i-1]
            out_channels = num_channels[i]
            blocks += [ElaticTemporalBlock1(in_channels, out_channels, kernel_size, stride=1, dilation=dilation_size,
                                     padding=(kernel_size-1) * dilation_size, dropout=dropout)]
